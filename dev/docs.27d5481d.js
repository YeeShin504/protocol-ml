...
a @2 < "left label @2"
b @5 > "right label @5"`;CodeMirror.commands.autocomplete=function(e){e.showHint({hint:CodeMirror.hint["protocol-ml"]})};let $=window.CodeMirror(document.getElementById("editor"),{value:y,mode:"protocol-ml",lineNumbers:!0,autofocus:!0,tabSize:2,indentUnit:2,indentWithTabs:!1,viewportMargin:1/0,extraKeys:{"Ctrl-Space":"autocomplete","Ctrl-/":"toggleComment","Cmd-/":"toggleComment",Tab:e=>{if(e.state.completionActive)e.state.completionActive.pick();else{let t=e.getCursor(),i=e.getLine(t.line).slice(0,t.ch);if(/^\s*$/.test(i))return CodeMirror.Pass;e.execCommand("autocomplete")}}},hintOptions:{direction:"above",completeSingle:!1,alignWithWord:!0}});$.on("inputRead",(e,t)=>{" "!==t.text[0]&&"\n"!==t.text[0]&&e.showHint({hint:CodeMirror.hint["protocol-ml"]})});let w="";function k(){let e=$.getValue();try{let o=function(e){let n={...t},o=[],r=[],l=0;for(let s of e.split("\n")){if(!(s=s.replace(/("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')|\/\/.*$/g,(e,t)=>t||"").trim()))continue;if(s.startsWith("def ")){let e=s.match(/^def\s+([\w.]+)\s+(.+)$/);if(!e)continue;let o=e[1],r=e[2],l=o.toLowerCase(),d=Object.keys(t).find(e=>e.toLowerCase()===l);if(d){let e=t[d];if("number"==typeof e){let e=a(r);null!==e&&(n[d]=e)}else if("boolean"==typeof e){let e=function(e){let t=i(e).toLowerCase();return"true"===t||"false"!==t&&null}(r);null!==e&&(n[d]=e)}else"string"==typeof e&&(n[d]=i(r))}continue}if(s.startsWith("participant ")){let e=s.match(/^participant\s+(?:(?:"([^"]+)")|(\S+))\s+(\w+)/);if(!e)continue;let t=e[1]||e[2],i=e[3];o.push({name:t,alias:i,column:o.length});continue}let e=s.match(/^(\w+)(?:\s*@([\d.%px]+))?\s*([<>])\s*"(.+)"/);if(e){l++,r.push({type:"annotation",participant:e[1],height:e[2]?a(e[2])??void 0:void 0,side:"<"===e[3]?"left":"right",text:e[4]});continue}let d=s.match(/^(\w+)(?:\s*@([\d.%px]+))?\s*(?:\[([\d.]+)\])?\s*(->|=>|~>|-x)(?:\[([\d.]+)\])?\s*(?:@([\d.%px]+)\s+)?(\w+)(?:\s*@([\d.%px]+))?(?:\s*:\s*"(.+)")?/);if(d){let e={"->":"normal","=>":"thick","~>":"corrupt","-x":"dropped"}[d[4]],t=d[3]?parseFloat(d[3]):void 0,i=d[5]?parseFloat(d[5]):void 0;"thick"===e&&(void 0!==t&&void 0===i?i=t:void 0!==i&&void 0===t&&(t=1)),r.push({type:"arrow",from:d[1],start:d[2]?a(d[2])??void 0:void 0,arrowType:e,thicknessStart:t,thicknessEnd:i,to:d[7],end:d[6]||d[8]?a(d[6]||d[8])??void 0:void 0,label:d[9]});continue}}return{settings:n,participants:o,actions:r,numAnnotations:l}}(e);w=function(e){let t,i,{settings:a,width:n,height:o,draws:r}=function(e){let{settings:t,participants:i,actions:a,numAnnotations:n}=e,o=(t.showTimeTicks?`Time ${t.timeUnit}`:"").length*t.participantFontSize*.3,r=(i[0]?.name||"").length*t.participantFontSize*.3,l=t.paddingX+Math.max(o,40),s=o+r+20,d=n>0?t.annotationWidth+t.labelOffset:0,p=t.showTimeTicks?Math.max(s,d):20+d,c=t.showTimeTicks?l+p:t.paddingX+p,m=c+t.participantSpacing*(i.length-1)+Math.max((i[i.length-1]?.name||"").length*t.participantFontSize*.3,d)+t.paddingX,x=t.paddingY+t.participantLabelHeight+t.timeTickInterval,h=new Map;{let e=c;for(let a of i)h.set(a.alias,e),e+=t.participantSpacing}let u=0,f=0,g=[];for(let e of a)switch(e.type){case"arrow":let i=u,a=u+1;void 0!==e.start&&(i=u=e.start,a=u+1),void 0!==e.end&&(a=e.end);let n="thick"===e.arrowType?e.thicknessStart??t.thickArrowThickness:0,o="thick"===e.arrowType?e.thicknessEnd??t.thickArrowThickness:0,r=n*t.timeTickInterval,l=o*t.timeTickInterval;g.push({type:e.type,x1:h.get(e.from),y1:x+i*t.timeTickInterval,x2:h.get(e.to),y2:x+a*t.timeTickInterval,arrowType:e.arrowType,thicknessStart:r,thicknessEnd:l,label:e.label}),f=Math.max(f,i+n,a+o),u++;break;case"annotation":let s=u;void 0!==e.height&&(s=e.height),g.push({type:e.type,x:h.get(e.participant)-("left"==e.side?1:-1)*t.labelOffset,y:x+s*t.timeTickInterval,align:"left"==e.side?"right":"left",text:e.text}),f=Math.max(f,s)}let y=x-t.timeTickInterval,$=Math.ceil(f);x+=($+1)*t.timeTickInterval;let w=i.map(e=>({type:"participant",x:h.get(e.alias),y1:y,y2:x,name:e.name}));if(t.showTimeTicks||t.showGrid){let e=Math.max(1,t.timeTickStep);for(let i=0;i<=$;i+=e)g.push({type:"tick",y:y+t.timeTickInterval+i*t.timeTickInterval,label:`${i}`})}return t.showTimeTicks&&g.push({type:"timeAxis",x:l,y1:y,y2:x}),x+=t.paddingY,{settings:t,width:m,height:x,draws:[...w,...g]}}(e),l=[];l.push(`<svg class="protocol-diagram" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${n} ${o}" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">`),l.push('<rect width="100%" height="100%" fill="#333333"/>');let s=r.filter(e=>"tick"===e.type),d=r.find(e=>"timeAxis"===e.type);for(let e of(a.showGrid&&l.push(s.map(e=>`<line stroke="#aaaaaa" stroke-dasharray="2" x1="0" y1="${e.y}" x2="${n}" y2="${e.y}" />`).join("\n")),a.showTimeTicks&&d&&l.push((t=`
<text
  x="${d.x}"
...
<line stroke="#aaaaaa" x1="${i}" y1="${a}" x2="${i}" y2="${n}" />`}(a,e));break;case"arrow":l.push(function(e,t){let{x1:i,y1:a,x2:n,y2:o,arrowType:r,thicknessStart:l,thicknessEnd:s,label:d}=t,p=n-i,c=o-a,m=Math.hypot(p,c),x=p/m,h=c/m,u=-h,f="",g=n-x*e.arrowHeadSize,y=o-h*e.arrowHeadSize,$=g+u*e.arrowHeadSize*.5,w=y+x*e.arrowHeadSize*.5,k=g-u*e.arrowHeadSize*.5,S=y-x*e.arrowHeadSize*.5;switch(r){case"normal":f=`
<g stroke="white" stroke-width="2" fill="none" stroke-linecap="round">
  <line x1="${i}" y1="${a}" x2="${n}" y2="${o}" />
  <line x1="${n}" y1="${o}" x2="${$}" y2="${w}" />
  <line x1="${n}" y1="${o}" x2="${k}" y2="${S}" />
</g>`;break;case"dropped":let b=i+x*m*e.dropStartRatio,v=a+h*m*e.dropStartRatio;f=`
<g stroke="white" stroke-width="2" fill="none" stroke-linecap="round">
  <line x1="${i}" y1="${a}" x2="${b}" y2="${v}" />
  <line stroke="red" x1="${b-.5*e.dropCrossSize}" y1="${v-.5*e.dropCrossSize}" x2="${b+.5*e.dropCrossSize}" y2="${v+.5*e.dropCrossSize}" />
  <line stroke="red" x1="${b-.5*e.dropCrossSize}" y1="${v+.5*e.dropCrossSize}" x2="${b+.5*e.dropCrossSize}" y2="${v-.5*e.dropCrossSize}" />
</g>`;break;case"corrupt":let T=i+x*m*e.corruptStartRatio,C=a+h*m*e.corruptStartRatio,z=(m*(1-e.corruptStartRatio)-2*e.arrowHeadSize)/(4*e.squiggleCount),L="",M=T,E=C;for(let t=0;t<e.squiggleCount;t++)M+=x*z,E+=h*z,L+=`L ${M-u*e.squiggleSize*.5} ${E-x*e.squiggleSize*.5} `,M+=2*x*z,E+=2*h*z,L+=`L ${M+u*e.squiggleSize*.5} ${E+x*e.squiggleSize*.5} `,M+=x*z,E+=h*z;M+=x*z,E+=h*z,L+=`L ${M} ${E}`,f=`
<g stroke="white" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <line x1="${i}" y1="${a}" x2="${T}" y2="${C}" />
  <path stroke="orange" d="M ${T} ${C} ${L} L ${g} ${y}" />
  <line stroke="orange" x1="${g}" y1="${y}" x2="${n}" y2="${o}" />
  <line stroke="orange" x1="${n}" y1="${o}" x2="${$}" y2="${w}" />
  <line stroke="orange" x1="${n}" y1="${o}" x2="${k}" y2="${S}" />
</g>`;break;case"thick":let H=n-i,I=o+s-(a+l),F=Math.hypot(H,I),N=H/F,R=I/F,B=-R,A=n-N*e.arrowHeadSize,W=o+s-R*e.arrowHeadSize,q=A+B*e.arrowHeadSize*.5,G=W+N*e.arrowHeadSize*.5,O=A-B*e.arrowHeadSize*.5,P=W-N*e.arrowHeadSize*.5;f=`
<g stroke="white" stroke-width="2" fill="none" stroke-linecap="round">
  <polygon stroke="none" fill="#ffffff44" points="${i},${a} ${n},${o} ${n},${o+s} ${i},${a+l}" />
  <line x1="${i}" y1="${a}" x2="${n}" y2="${o}" />
  <line x1="${n}" y1="${o}" x2="${$}" y2="${w}" />
  <line x1="${n}" y1="${o}" x2="${k}" y2="${S}" />
  <line x1="${i}" y1="${a+l}" x2="${n}" y2="${o+s}" />
  <line x1="${n}" y1="${o+s}" x2="${q}" y2="${G}" />
  <line x1="${n}" y1="${o+s}" x2="${O}" y2="${P}" />
</g>`}let U="";if(d){let t=(a+o)/2,m=e.labelOffset,h=1,f=180*Math.atan2(c,p)/Math.PI;(f>90||f<-90)&&(f+=180,m*=h=-1);let g=(i+n)/2-u*m,y="thick"===r?t+h*x*(l+s)/4:t-x*m;U=`
<text
  x="${g}"
  y="${y}"
  text-anchor="middle"
...
//# sourceMappingURL=docs.27d5481d.js.map
