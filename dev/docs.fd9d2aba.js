...
<line stroke="#aaaaaa" x1="${i}" y1="${a}" x2="${i}" y2="${n}" />`}(a,e));break;case"arrow":l.push(function(e,t){let{x1:i,y1:a,x2:n,y2:o,arrowType:r,thickness:l,label:s}=t,d=n-i,p=o-a,c=Math.hypot(d,p),m=d/c,x=p/c,u=-x,h="",f=n-m*e.arrowHeadSize,g=o-x*e.arrowHeadSize,y=f+u*e.arrowHeadSize*.5,$=g+m*e.arrowHeadSize*.5,w=f-u*e.arrowHeadSize*.5,k=g-m*e.arrowHeadSize*.5;switch(r){case"normal":h=`
...
</g>`;break;case"dropped":let b=i+m*c*e.dropStartRatio,S=a+x*c*e.dropStartRatio;h=`
<g stroke="white" stroke-width="2" fill="none" stroke-linecap="round">
  <line x1="${i}" y1="${a}" x2="${b}" y2="${S}" />
  <line stroke="red" x1="${b-.5*e.dropCrossSize}" y1="${S-.5*e.dropCrossSize}" x2="${b+.5*e.dropCrossSize}" y2="${S+.5*e.dropCrossSize}" />
  <line stroke="red" x1="${b-.5*e.dropCrossSize}" y1="${S+.5*e.dropCrossSize}" x2="${b+.5*e.dropCrossSize}" y2="${S-.5*e.dropCrossSize}" />
</g>`;break;case"corrupt":let v=i+m*c*e.corruptStartRatio,C=a+x*c*e.corruptStartRatio,T=(c*(1-e.corruptStartRatio)-2*e.arrowHeadSize)/(4*e.squiggleCount),z="",L=v,M=C;for(let t=0;t<e.squiggleCount;t++)L+=m*T,M+=x*T,z+=`L ${L-u*e.squiggleSize*.5} ${M-m*e.squiggleSize*.5} `,L+=2*m*T,M+=2*x*T,z+=`L ${L+u*e.squiggleSize*.5} ${M+m*e.squiggleSize*.5} `,L+=m*T,M+=x*T;L+=m*T,M+=x*T,z+=`L ${L} ${M}`,h=`
<g stroke="white" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <line x1="${i}" y1="${a}" x2="${v}" y2="${C}" />
  <path stroke="orange" d="M ${v} ${C} ${z} L ${f} ${g}" />
...
</g>`}let E="";if(s){let t=(a+o)/2,c=e.labelOffset,x=1,h=180*Math.atan2(p,d)/Math.PI;(h>90||h<-90)&&(h+=180,c*=x=-1);let f=(i+n)/2-u*c,g="thick"===r?t+x*m*l/2:t-m*c;E=`
<text
  x="${f}"
  y="${g}"
  text-anchor="middle"
